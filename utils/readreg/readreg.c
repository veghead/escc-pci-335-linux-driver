/* $Id: readreg.c,v 1.2 2003/11/11 21:01:54 carl Exp $ */
/*
Copyright(c) 2003, Commtech, Inc.
readreg.c -- user mode function to read an 82532 register

usage:
 readreg port register

 The port can be any valid escc port (0,1)
 The register is assumed to be a hex value (00 - 3f)
...
int main(int argc, char *argv[])
{
	...
	{
		printf("usage:\n%s port register\n", argv[0]);
		printf("port appended to /dev/escc\n");
		printf("register as a hex value\n");
		exit(1);
	}
	port = atoi(argv[1]);
	sscanf(argv[2], "%x", &reg.port);
	...
	if (ioctl(escc, ESCC_READ_REGISTER, &reg) == -1)
		perror(NULL);
	printf("%s reg:%x ==> %x\n", nbuf, reg.port, reg.data);
	close(escc);
	return 0;
}
